        ...
        <aside className="hidden w-64 shrink-0 flex-col justify-between border-r border-[#e8e1d3] bg-[#fffdf8] p-6 lg:flex">
          <div className="space-y-8">
            ...
            <nav className="space-y-1.5">
              ...
              <button className="relative flex w-full items-center gap-3 rounded-xl px-4 py-3 text-xs font-bold text-[#9b5416]"><span className="absolute inset-0 rounded-xl border border-[#f7d786] bg-[#fff0c9] shadow-sm" /><Brain className="relative z-10 h-4 w-4 text-[#e67b1f]" /><span className="relative z-10">Insights</span></button>
            </nav>
          </div>
          <div className="space-y-3.5"><div className="space-y-3 rounded-2xl border border-[#e8e1d3] bg-white p-4 shadow-sm"><div className="flex items-center justify-between text-xs font-semibold text-slate-700"><span className="flex items-center gap-2"><Flame className="h-4 w-4 fill-amber-500 text-amber-500" />7 Day Streak</span><span className="rounded-full border border-[#f6d89b] bg-[#fff1d5] px-2 py-0.5 font-mono text-[10px] font-bold text-[#c56a17]">Active</span></div><div className="flex items-center justify-between border-t border-[#eee7da] pt-2.5 text-xs text-slate-500"><span>Focus area</span><span className="font-mono text-sm font-bold text-[#d97717]">Insights</span></div></div><div className="flex items-center justify-between rounded-2xl border border-[#e8e1d3] bg-white p-2.5"><div className="flex items-center gap-2.5"><div className="grid h-7 w-7 place-items-center rounded-full bg-gradient-to-tr from-amber-500 to-amber-300 text-xs font-black text-slate-950">P</div><span className="text-xs font-bold text-slate-700">Pattern learner</span></div><span className="h-2 w-2 rounded-full bg-emerald-400" /></div></div>
        </aside>
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="flex shrink-0 items-center justify-between border-b border-[#e8e1d3] bg-[#fffdf8]/90 px-5 py-4 backdrop-blur-xl sm:px-8"><div><h1 className="flex items-center gap-2 text-lg font-black tracking-tight text-[#17263a]"><Brain className="h-5 w-5 text-[#e67b1f]" />Mistake Intelligence</h1><p className="text-xs font-medium text-slate-400">Turn failed attempts into a sharper practice plan</p></div><button onClick={() => router.push('/interview-arena')} className="hidden items-center gap-1.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-400 px-4 py-2 text-xs font-black text-slate-950 shadow-lg shadow-amber-500/20 sm:flex"><Award className="h-4 w-4" />Launch Assessment</button></header>
          <main className="min-h-0 flex-1 overflow-y-auto px-4 pb-16 pt-5 sm:px-8 sm:pt-7">
            {/* Banner */}
            <div className="mx-auto max-w-5xl space-y-2 rounded-2xl border border-[#e8e1d3] bg-[#fffdf8] p-5 shadow-sm">
            ...
            {loading ? (
              ...
              <div className="space-y-3 rounded-2xl border border-[#e8e1d3] bg-white p-8 text-center text-xs text-slate-500">
                ...
                <button onClick={() => window.location.reload()} className="rounded-xl bg-[#17263a] px-4 py-2 text-xs font-bold text-amber-300 shadow-sm">Retry analytics</button>
              </div>
            ) : !data ? (
              <div className="mx-auto max-w-5xl rounded-2xl border border-[#e8e1d3] bg-[#fffdf8] p-8 text-center text-xs text-slate-500">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(data.mistakeCategories).map(([category, count], idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between rounded-2xl border border-[#e8e1d3] bg-[#fffdf8] p-4 shadow-sm"
                      >
                        ...
                        <span className="rounded-lg border border-[#f6d89b] bg-[#fff1d5] px-3 py-1 text-xs font-mono font-bold text-[#a85b17]">
                          {count} Errors
                        </span>
                      </div>
                    ))}
                ...
                <div className="rounded-2xl border border-[#e8e1d3] bg-[#fffdf8] p-5 font-sans shadow-sm">
                  ...
                  <div className="space-y-3">
                    {data.recentVerdictHistory.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between rounded-xl border border-[#e8e1d3] bg-[#f8f5ed] p-3.5 text-xs"
                      >
